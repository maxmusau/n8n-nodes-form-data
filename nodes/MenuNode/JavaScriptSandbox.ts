...
export class JavaScriptSandbox extends Sandbox {
	...
	async runCodeEachItem(): Promise<INodeExecutionData | undefined> {
		...
		// return this.$input.all().map((item) => {
		return this.$input.all().map((item: Array<string>) => {
			const output: INodeExecutionData = {
				json: {},
				binary: {},
			};

			Object.assign(output.json, output.json);

			// Object.assign(output.binary, item.binary);
			if (output.binary) {
				Object.assign(output.binary, output.binary);
			}
